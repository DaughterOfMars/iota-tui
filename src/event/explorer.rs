...
use crate::app::{App, InputMode, LookupAction};
...
pub fn handle_explorer_key(app: &mut App, key: KeyEvent) {
    ...
    match key.code {
        ...
        KeyCode::End => {
            if !app.explorer.search_results.is_empty() {
                app.explorer.search_selected = app.explorer.search_results.len().saturating_sub(1);
        ...
        // Address lookup pagination: next
        KeyCode::Char(']')
            if app.explorer.search_results.is_empty()
                ...
                && (app.explorer.lookup_obj_has_next || app.explorer.lookup_tx_has_next) =>
        {
            app.explorer
                .lookup_obj_cursors
        ...
        // Address lookup pagination: prev
        KeyCode::Char('[')
            if app.explorer.search_results.is_empty()
                && app.explorer.lookup_address.is_some()
                && matches!(
...
fn lookup_cursor_up(app: &mut App) {
    ...
    } else if app.explorer.lookup_section > 0 {
...
fn lookup_cursor_down(app: &mut App) {
    ...
    if app.explorer.lookup_depth == 1 {
        ...
        if app.explorer.lookup_field_idx + 1 < section.fields.len() {
            app.explorer.lookup_field_idx += 1;
        } else if app.explorer.lookup_section + 1 < sections.len() {
            app.explorer.lookup_section += 1;
            app.explorer.lookup_depth = 0;
            app.explorer.lookup_field_idx = 0;
        }
...
fn lookup_cursor_left(app: &mut App) {
    if app.explorer.lookup_depth == 1 {
        ...
        && let Some(s) = result.sections_mut().get_mut(app.explorer.lookup_section)
    {
        s.collapsed = true;
    }
}
